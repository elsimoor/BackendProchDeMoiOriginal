...
exports.paymentResolvers = {
    Query: {
        ...
        payments: async (_parent, { businessId }) => {
            /**
            ...
            try {
                ...
                const restaurant = await RestaurantModel_1.default.findById(businessId).select('clientId').lean();
            ...
            try {
                ...
                const salon = await SalonModel_1.default.findById(businessId).select('clientId').lean();
            ...
            return PaymentModel_1.default.find({ businessId: { $in: uniqueIds } }).sort({ createdAt: -1 });
        },
        // Fetch a single payment by identifier.
        payment: async (_parent, { id }) => {
    ...
    Mutation: {
        ...
        createPaymentSession: async (_parent, { input }) => {
            const { reservationId, successUrl, cancelUrl } = input;
            // Look up the reservation.  Throw if not found.
            const reservation = await ReservationModel_1.default.findById(reservationId);
            if (!reservation) {
            ...
            if (!amount || amount <= 0) {
                const invoice = await InvoiceModel_1.default.findOne({ reservationId: reservation._id });
                if (invoice) {
            ...
            try {
                ...
                if (type === 'restaurant') {
                    ...
                    const restaurant = await RestaurantModel_1.default.findOne({ clientId: businessId });
                    const cur = restaurant?.settings?.currency;
                    if (cur)
                        currency = cur;
                }
                else if (type === 'salon') {
                    const salon = await SalonModel_1.default.findOne({ clientId: businessId });
            ...
            // currency used.
            const paymentRecord = new PaymentModel_1.default({
    ...
    Payment: {
        ...
        invoice: async (parent) => {
            if (parent.invoiceId) {
                return InvoiceModel_1.default.findById(parent.invoiceId);
            }
            return null;
        },
    },
