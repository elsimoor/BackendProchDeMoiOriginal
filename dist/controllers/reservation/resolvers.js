...
exports.reservationResolvers = {
    ...
    Mutation: {
         ...
         */
        cancelHotelReservation: async (_parent, { id, cancelledBy }) => {
            const reservation = await ReservationModel_1.default.findById(id);
            if (!reservation) {
                throw new Error('Reservation not found');
            }
            ...
            if (reservation.businessType === 'hotel' &&
                ...
                const policies = await CancellationPolicyModel_1.default.find({ businessId: reservation.businessId }).sort({ daysBefore: -1 });
            ...
            if (cancelledBy?.toLowerCase() === 'user' && refundAmount > 0) {
                try {
                    ...
                    const payment = await PaymentModel_1.default.findOne({ reservationId: id, status: 'paid' });
    ...
    Reservation: {
        ...
        currency: async (parent) => {
            ...
            try {
                ...
                if (type === 'restaurant') {
                    const restaurant = await RestaurantModel_1.default.findOne({ clientId: parent.businessId });
                    ...
                    const salon = await SalonModel_1.default.findOne({ clientId: parent.businessId });
                    ...
                    if (parent.roomId) {
                        ...
                        const room = await RoomModel_1.default.findById(parent.roomId).populate('hotelId');
