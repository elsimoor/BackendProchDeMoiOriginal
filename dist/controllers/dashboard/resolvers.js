...
exports.dashboardResolvers = {
    Query: {
        dashboardMetrics: async (_, { restaurantId, from, to }) => {
            ...
            let startDate = moment_1.default.utc().startOf('day');
            let endDate = moment_1.default.utc().endOf('day');
            if (from && to) {
        ...
        availability: async (_, { restaurantId, date, partySize }) => {
            ...
            reservations.forEach((r) => {
                ...
                try {
                    ...
                    const normalized = moment_1.default.utc(`${date}T${r.time}`).format('HH:mm');
