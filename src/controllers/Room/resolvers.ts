...
export const roomResolvers: IResolvers<unknown, Context> = {
  Query: {
     ...
     */
    availableRoomsAllHotels: async (
      _parent,
      { checkIn, checkOut, adults, children }: { checkIn: string; checkOut: string; adults: number; children: number }
    ) => {
      const start = new Date(checkIn);
      const end = new Date(checkOut);
      const totalGuests = adults + children;
      ...
      const rooms = await RoomModel.find({ isActive: true, status: 'available', capacity: { $gte: totalGuests } }).populate('hotelId');
      if (!rooms || rooms.length === 0) return [];
